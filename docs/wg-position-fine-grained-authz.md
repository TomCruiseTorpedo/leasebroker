...
| Ungoverned traffic is still recorded | a call forwarded with no lease check (no capability mapped to the tool name) emits a distinct `passthrough` event, never a `use` — the log does not present governed traffic as the whole picture | shipped, tested |
